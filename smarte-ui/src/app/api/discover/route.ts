...
import { discoverContacts, discoverCompanies } from "@/lib/smarte-client";
...
export async function POST(req: NextRequest) {
  ...
  switch (type) {
    case "contacts":
      result = await discoverContacts(apiKey, params as Parameters<typeof discoverContacts>[1]);
      break;
    case "companies":
      result = await discoverCompanies(apiKey, params as Parameters<typeof discoverCompanies>[1]);
      break;
    default:
      return NextResponse.json(
        { error: true, code: "UNKNOWN_TYPE", message: `Unknown discover type: ${type}` },
        { status: 400 }
      );
  }
